...
int main(int ac, char *av[])
{
    ...
    /* 옵션 처리 */
    // ./cp1 cp1.c cp1.c 명령어나 
    // ./cp1 -i cp1.c copyed.c 명령어를 입력하니까.
    // 해당 명령어들은 각각 명령줄 인자가 3개, 4개이기때문. copy에 관한 명령줄 인자들은 3개 혹은 4개 이외엔 없다.
    if (ac < 3 || ac > 4) {
    ...
    if (interactive && access(av[2], F_OK) == 0) {
        ...
        while (1) {
            int c = getchar();
            if (c == 'y' || c == 'Y') {
                break;  // 'y' 또는 'Y' 입력 시 덮어쓰기 진행
            } else if (c == 'n' || c == 'N') {
                fprintf(stderr, "복사를 취소했습니다.\n");
                exit(1);  // 'n' 또는 'N' 입력 시 종료
            }
        }
    }
